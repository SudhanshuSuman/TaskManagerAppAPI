...
router.get('/tasks', auth, async (req, res) => {
    ...
    try {
        // const tasks = await Task.find({ belongsTo: req.user._id })           // OR:
...
router.get('/tasks/:id', auth, async (req, res) => {
    try {
        const task = await Task.findOne({ _id: req.params.id, belongsTo: req.user._id })
        if(!task) {
...
router.delete('/tasks/:id', auth, async (req, res) => {
    try {
        const task = await Task.findOneAndDelete({ _id: req.params.id, belongsTo: req.user._id })
        if(!task) {
            return res.status(404).send()
        }
        res.send(task)
